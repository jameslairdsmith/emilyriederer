...
Last winter, I attended a holiday party at a "paint-and-sip" venue. For those unfamiliar, "paint-and-sip" is a semi-trendy cottage industry offering boutique events of music, wine, and a guided painting activity. For example, my group painted sasquatch on a snowy winter's eve:

![A very bad painting of a sasquatch in a forest](/img/rtistic-files/IMG_1089.jpg)

As often happens, this completely unrelated thing got me thinking about R. What was it that made coming together fun when we lacked the talent and experience to do well, when it almost surely wouldn't turn out very well, and when "failure" would be very visible and undeniable? Many of these same dimensions (novelty, risk, and visibility) often seem to inhibit new coders, but somehow they were core to the value proposition of the paint-and-sip business. 
...
- **The objective wasn't too serious**. Complete and utter failure was low stakes. No balls would be dropped. No meetings would be postponed. No deadlines would pass by. There was no dire consequence of trying to paint a sasquatch.
- **The ability to succeed wasn't correlated with anything important**. Hopefully, no one is going to measure their self-worth by sasquatch painting. In contrast, sometimes people seem to associate technical skills and abilities as more of a measure of their worth or intelligence. This increses the cost of "failure" and disuades exploration. 
- **The playing field was mostly level**. As far as I know, there are no experts in acryllic sasquatch painting.
...
![Rtistic hex logo](/img/rtistic-files/logo.png)
...
Much of the boilerplate code already exists, so partipants are less likely to get caught up on some cryptic error message. Off-the-shelf, the project's file structure looks like what's shown below. Files prefixed with `my-` are open to edits by each team. 
...
- `scale_(color/colour/fill)_opinionated_rtistic(palette = "test")`: Discrete palette to map to subjectively coded "good"/"bad"/"neutral" column
...
To "level up", participants can also use `roxygen2` syntax to add documentation about their theme and showcase it by editing the [`my-gg-theme-vignette.Rmd` template](https://github.com/emilyriederer/Rtistic/blob/master/scratchpad/gg-theme-demo.Rmd). Adding actual `ggplot2` and RMarkdown themes is slightly more advanced, but the core design of participants altering templates still holds.

You can try all this out for yourself on [RStudio Cloud](https://rstudio.cloud/project/363105). Simply resave the `my-gg-palette.R` file, define new palette colors (named `{xxx}_{pal/pal_op/pal_cont/pal_div}`), rebuild the package, open the `vignettes/my-gg-theme-vignette.Rmd` file, change `palette = "test"` to `palette = "{xxx}"` throughout, and reknit!
...
Again, try this out on [RStudio Cloud](https://rstudio.cloud/project/363105) by opening up the `scratchpad/gg-theme-demo.Rmd` and playing around.
