...
```{r echo = FALSE}
text <- RCurl::getURL("https://raw.githubusercontent.com/emilyriederer/Rtistic/master/R/my-gg-palette.R")
cat(text)
```
...
The `get_rtistic_palettes()` function scans the package's namespace for anything ending in `_pal` to help users learn about all the available options^[This allows discoverbility without having to maintain any centralized list of palettes since, for logistical reasons, `Rtistic` is designed to be completely modular.]:
